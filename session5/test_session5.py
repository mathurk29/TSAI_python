...
import math
import time
from session5 import squared_power_list, time_it
...
def test_session5_readme_proper_description():
    """ 
    Verifies whether the README.md explains all the functions which are present in session5.py. 
    """
...
def test_session5_indentations():
    """
    Verifies if the session5.py file follows the PEP8 guidelines. 
    """
...
def test_session5_time_it_print():
    """ 
    Test to verify whether the time_it() of session5.py gives out proper average run time per call for print() function.
    """
    res = time_it(print,repetitions=5)
    print(f'res is {res}')
    assert res >= 0.0  , "time_it can't time print function"


def test_session5_time_it_squared_power_list():
    """
    Test to verify whether the time_it() of session5.py gives out proper average run time per call for squared_power_list() function.
    """
    res = time_it(fn = squared_power_list, repetitions=5, number = 5)
    assert res >= 0.0 , "time_it can't time squared_power_list function"


def test_session5_time_it_polygon_area():
    """
    Test to verify whether the time_it() of session5.py gives out proper average run time per call for polygon_area() function.
    """
    res = time_it(fn = polygon_area, repetitions=5, length = 5)
    assert res >= 0.0 , "time_it can't time polygon_area function"


def test_session5_time_it_temp_converter():
    """
    Test to verify whether the time_it() of session5.py gives out proper average run time per call for temp_converter() function.
    """
    res = time_it(fn = temp_converter, repetitions=5, temp = 5)
    assert res >= 0.0 , "time_it can't time temp_converter function"


def test_session5_time_it_speed_converter():
    """
    Test to verify whether the time_it() of session5.py gives out proper average run time per call for speed_converter() function.
    """
    res = time_it(fn = speed_converter, repetitions=5, speed = 5)
    assert res >= 0.0 , "time_it can't time speed_converter function"
...
def test_session5_polygon_area():
    ...
    with pytest.raises(TypeError, match=r".*missing 1 required positional argument: 'length'*"):
...
def test_session5_polygon_area_sides_values():
    ...
    with pytest.raises(ValueError,match=r'should be between 3 to 6 both inclusive'):
...
def test_session5_temp_converter():
    ...
    with pytest.raises(TypeError, match = r"missing 1 required positional argument: 'temp'"):
